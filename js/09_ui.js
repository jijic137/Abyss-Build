...
(function () {
  ...
  UI.renderCoverBg = function () {
    ...
    try {
      ...
      // 深渊裂隙：从底部向上张开的不规则锯齿光缝（上方顶点锐利，底部宽开）
      var cx = vw / 2;
      var riftTop = vh * 0.62;
      var riftBot = vh + 30;
      var seg = 36;
      ...
      // 裂隙外圈光晕（向两侧扩散，宽广）
      var bg2 = c.createRadialGradient(cx, vh * 0.74, 0, cx, vh * 0.74, Math.max(vw, vh) * 0.26);
      bg2.addColorStop(0, 'rgba(170,220,255,.28)');
      bg2.addColorStop(1, 'rgba(170,220,255,0)');
      c.fillStyle = bg2;
      c.fillRect(0, 0, vw, vh);

      // 裂隙顶部扩散光晕（让标题底部像从光中升起）
      var halo = c.createRadialGradient(cx, riftTop, 0, cx, riftTop, vw * 0.18);
